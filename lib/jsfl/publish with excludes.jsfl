...
function exportWithExcludes()
{
	 ...
	 */
	function getDocumentPath(){return document.path?fixPath(document.path.replace(/\\/g,'/').split('/').slice(0,-1).join('/'))+'/':null;}
	...
	if(getDocumentPath())
	{
		...
		if(contents)
		{
			...
			if(excludedClasses.length > 0)
			{
				...
				//get global paths.
				as3GlobalPath = fl.as3PackagePaths.replace(".;$(AppConfig)/ActionScript 3.0/Classes","").split(";");
				for(var f=0;f<as3GlobalPath.length;f++)
				{
					if(as3GlobalPath[f] == "" || as3GlobalPath[f] == " ") continue;
					var src = as3GlobalPath[f].toString();
					//fl.trace(fixPath(src));
					src = src.replace(/\:/g,"/");
					src = src.replace(/\/\//g,"/");
					src = "file:///"+escape(src);
					as3PackagePaths.push(src);
					//fl.trace(src);
					//fl.trace("ITEM");
					//as3GlobalPath[f] = "file:///"+src;
					//fl.trace(as3GlobalPath[f]);
				}
				//as3PackagePaths.concat(as3GlobalPath);
				fl.trace(as3PackagePaths);
				return;
